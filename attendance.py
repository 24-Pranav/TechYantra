...
class Attendance:
    ...
    def __init__(self,root):
        ...
        f_lb1.place(x=0, y=0, width=770, height=130)

        # set image as lable
        f_lb2 = Label(self.root, image=self.photoimg)
        f_lb2.place(x=770, y=0, width=770, height=130)

        # backgorund image
        bg1 = Image.open(r"Resources\colorful.png")
        bg1 = bg1.resize((1565, 900), Image.LANCZOS)
        self.photobg1 = ImageTk.PhotoImage(bg1)

        # set image as lable
        bg_img = Label(self.root, image=self.photobg1)
        bg_img.place(x=0, y=130, width=1565, height=900)

        # title section
        title_lb1 = Label(bg_img, text="Welcome to Attendance Panel", font=(
            "verdana", 30, "bold"), bg="white", fg="navyblue")
        title_lb1.place(x=0, y=0, width=1565, height=45)

        # ========================Section Creating==================================

        # Creating Frame
        main_frame = Frame(bg_img, bd=2, bg="white")  # bd mean border
        main_frame.place(x=5, y=55, width=1520, height=595)

        # Left Label Frame
        left_frame = LabelFrame(main_frame, bd=2, bg="lightgreen", relief=RIDGE,
                                text="Student Details", font=("verdana", 12, "bold"), fg="orange")
        left_frame.place(x=10, y=10, width=745, height=575)

        # ==================================Text boxes and Combo Boxes====================

        # Student id
        studentId_label = Label(
            left_frame, text="Std-ID:", font=("verdana", 12, "bold"), fg="navyblue", bg="lightgreen")
        ...
        student_roll_label.grid(row=0, column=2, padx=60, pady=5, sticky=W)

        student_roll_entry = ttk.Entry(
            left_frame, textvariable=self.var_roll, width=15, font=("verdana", 12, "bold"))
        student_roll_entry.grid(row=0, column=3, padx=0, pady=5, sticky=W)

        # Studnet Name
        student_name_label = Label(
            left_frame, text="Std-Name:", font=("verdana", 12, "bold"), fg="navyblue", bg="lightgreen")
        ...
        time_label = Label(left_frame, text="Time:", font=(
            "verdana", 12, "bold"), fg="navyblue", bg="lightgreen")
        time_label.grid(row=1, column=2, padx=60, pady=5, sticky=W)

        time_entry = ttk.Entry(
            left_frame, textvariable=self.var_time, width=15, font=("verdana", 12, "bold"))
        time_entry.grid(row=1, column=3, padx=0, pady=5, sticky=W)
        ...
        student_attend_label = Label(
            left_frame, text="Attend-status:", font=("verdana", 12, "bold"), fg="navyblue", bg="lightgreen")
        student_attend_label.grid(row=2, column=2, padx=60, pady=5, sticky=W)
        ...
        attend_combo.grid(row=2, column=3, padx=0, pady=5, sticky=W)

        # ===============================Table Sql Data View==========================
        table_frame = Frame(left_frame, bd=2, bg="white", relief=RIDGE)
        table_frame.place(x=10, y=100, width=720, height=390)
        ...
        self.attendanceReport_left = ttk.Treeview(table_frame, column=(
            "ID", "Roll_No", "Name", "Time", "Date", "Attend"), xscrollcommand=scroll_x.set,
        ...
        btn_frame.place(x=10, y=490, width=720, height=60)

        # Improt button
        save_btn = Button(btn_frame, command=self.importCsv, text="Import CSV", width=14, font=(
            "verdana", 12, "bold"), fg="white", bg="lightgreen")
        save_btn.grid(row=0, column=0, padx=10, pady=10, sticky=W)

        # Exprot button
        update_btn = Button(btn_frame, command=self.exportCsv, text="Export CSV", width=14, font=(
            "verdana", 12, "bold"), fg="white", bg="slateblue")
        update_btn.grid(row=0, column=1, padx=6, pady=8, sticky=W)

        # Update button
        del_btn = Button(btn_frame, command=self.action, text="Update", width=14, font=(
            "verdana", 12, "bold"), fg="white", bg="salmon")
        del_btn.grid(row=0, column=2, padx=6, pady=10, sticky=W)

        # reset button
        reset_btn = Button(btn_frame, command=self.reset_data, text="Reset", width=14, font=(
        ...
        # Right Label Frame
        right_frame = LabelFrame(main_frame, bd=2, bg="lightgreen", relief=RIDGE,
                                 text="Student Details", font=("verdana", 12, "bold"), fg="navyblue")
        right_frame.place(x=765, y=10, width=745, height=575)

        # ------------------------------  Table Frame  -------------------------------------------
        # Table Frame
        # Searching System in Right Label Frame
        table_frame = Frame(right_frame, bd=2, bg="white", relief=RIDGE)
        table_frame.place(x=10, y=90, width=720, height=460)
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattendance set std_id='"+self.var_id.get()+"',std_roll_no='"+self.var_roll.get()+"',std_name='"+self.var_name.get()+"',std_time='"+self.var_time.get()+"',std_date='"+self.var_date.get()+"',std_attendance='"+self.var_attend.get()+"' where std_id='"+self.var_id.get()+"'")
                else:
                    if not Update:
                        return
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = sqlite3.connect(r"database.db")
                    mycursor = conn.cursor() 
                    sql="delete from stdattendance where std_id='"+self.var_id.get()+"'"
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = sqlite3.connect(r"database.db")
                mycursor = conn.cursor()
                mycursor.execute("insert into stdattendance values('"+self.var_id.get()+"','"+self.var_roll.get()+"','"+self.var_name.get()+"','"+self.var_time.get()+"','"+self.var_date.get()+"','"+self.var_attend.get()+"')")
